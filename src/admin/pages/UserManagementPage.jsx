...
export default function UserManagementPage() {
    ...
    useEffect(() => {
        (async () => {
            setFetching(true);
            try {
                const res = await axios.get(API.baseURL, { headers: API.headers() });
                setUsers(res.data);
                setServerError("");
            } catch (err) {
    ...
    const handleCreate = async (e) => {
        ...
        try {
            ...
            const res = await axios.post(API.baseURL, payload, { headers: API.headers() });
    ...
    const handleToggleLock = async (id) => {
        try {
            const res = await axios.patch(`${API.baseURL}/${id}/toggle-status`, {}, { headers: API.headers() });
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete(`${API.baseURL}/${id}`, { headers: API.headers() });
    ...
    const handleResetPassword = async (e) => {
        ...
        setLoading(true);
        try {
            await axios.patch(`${API.baseURL}/${showReset.id}/reset-password`, { password: form.password }, { headers: API.headers() });
            setShowReset(null);
    ...
    const openEditModal = (user) => {
        setEditForm({
            ...
            password_hash: user.password_hash, status: user.status,
            student_id: user.student_id ?? "", class_name: user.class_name ?? "",
            date_of_birth: user.date_of_birth ?? "", faculty: user.faculty ?? "",
            gender: user.gender ?? "", major: user.major ?? "", phone_number: user.phone_number ?? "",
        });
        setShowEdit(user);
        setFormError("");
    };
    ...
    const handleUpdate = async (e) => {
        e.preventDefault();
        setFormError("");
        if (!editForm.full_name.trim() || !editForm.email.trim()) { setFormError("Full Name and Email are required"); return; }
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(editForm.email)) { setFormError("Invalid email format"); return; }
        setLoading(true);
        try {
            const payload = {
                full_name: editForm.full_name, email: editForm.email,
                role: editForm.role.toUpperCase(),
                date_of_birth: editForm.date_of_birth, faculty: editForm.faculty,
                gender: editForm.gender, phone_number: editForm.phone_number,
                student_id: editForm.role === "student" ? (editForm.student_id || null) : null,
                class_name: editForm.role === "student" ? (editForm.class_name || null) : null,
                major: editForm.role === "student" ? (editForm.major || null) : null,
            };
            const res = await axios.put(`${API.baseURL}/${showEdit.id}`, payload, { headers: API.headers() });
    ...
    const renderSelect = (value, onChange, options) => (
        <select value={value} onChange={onChange} className="w-full px-4 py-2.5 rounded-xl border border-zinc-200 text-sm outline-none transition-all duration-200 focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 bg-white">
            {options.map((opt) => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
        </select>
    );

    const renderRoleRadios = (currentRole, onChange) => (
        <div className="flex gap-2">
            {["student", "teacher"].map((r) => (
                <label key={r} className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border text-sm cursor-pointer transition-all duration-200 ${currentRole === r ? "border-indigo-300 bg-indigo-50 text-indigo-700" : "border-zinc-200 bg-white text-zinc-500 hover:border-zinc-300"}`}>
                    <input type="radio" value={r} checked={currentRole === r} onChange={onChange} className="sr-only" />
    ...
    const renderDetailRow = (label, value) => (
        <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-zinc-700">{value ?? <span className="text-zinc-300 italic">N/A</span>}</span>
        </div>
    );

    /** Shows only when role === "student" — fields exclusive to student profiles */
    const StudentOnlyFields = ({ formData, setter }) => {
    ...
    /** Shared fields rendered for all roles — gender, DOB, phone, faculty */
    const SharedFields = ({ formData, setter }) => {
        const updater = (field) => (e) => setter((p) => ({ ...p, [field]: e.target.value }));
        return (
            <div className="grid grid-cols-2 gap-3">
                <div>
                                    ...
                                    <tr key={u.id} className={`group transition-all duration-150 hover:bg-zinc-50/80 ${idx < filtered.length - 1 ? "border-b border-zinc-50" : ""}`}>
                                        ...
                                        <td className="px-5 py-4">
                                            <div className="space-y-1.5 min-w-[220px]">
                                                {renderDetailRow("ID", u.id)}
                                                {renderDetailRow("Role",
                                                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-medium ${u.role === "teacher" ? "bg-amber-50 text-amber-700" : "bg-indigo-50 text-indigo-700"}`}>
                                                        {u.role === "teacher" ? <GraduationCap className="w-2.5 h-2.5" /> : <User className="w-2.5 h-2.5" />}
                                                        {u.role === "teacher" ? "Teacher" : "Student"}
                                                    </span>
                                                )}
                                                {u.role === "student" && renderDetailRow("Student ID", u.student_id)}
                                                {u.role === "student" && renderDetailRow("Class", u.class_name || <span className="text-zinc-300 italic">—</span>)}
                                                {u.role === "student" && renderDetailRow("Major", u.major || <span className="text-zinc-300 italic">—</span>)}
                                                {renderDetailRow("Gender", u.gender)}
                                                {renderDetailRow("DOB", u.date_of_birth)}
                                                {renderDetailRow("Phone", u.phone_number || <span className="text-zinc-300 italic">—</span>)}
                                                {renderDetailRow("Faculty", u.faculty || <span className="text-zinc-300 italic">—</span>)}
                                                {renderDetailRow("Password Hash", u.password_hash || "********")}
                                                {renderDetailRow("Updated", u.updated_at)}
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-5 py-4 text-center text-xs text-zinc-400">{u.created_at}</td>
                                        <td className="px-5 py-4 text-right">
                                            <div className="flex items-center justify-end gap-0.5 opacity-0 group-hover:opacity-100 transition-all duration-200">
                                                ...
                                                <button onClick={() => handleToggleLock(u.id)} className={`p-2 rounded-lg transition-all duration-200 cursor-pointer ${u.status === "active" ? "text-zinc-300 hover:text-amber-500 hover:bg-amber-50" : "text-zinc-300 hover:text-emerald-500 hover:bg-emerald-50"}`} title={u.status === "active" ? "Lock" : "Unlock"}>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-white rounded-2xl shadow-2xl p-7 w-full max-w-lg z-10 border border-zinc-100 animate-in slide-in-from-bottom-4 duration-200">
                        ...
                        <form onSubmit={handleCreate} className="space-y-4">
                            ...
                            <div>
                                {renderLabel("Role")}
                                {renderRoleRadios(form.role, (e) => setForm((p) => ({ ...p, role: e.target.value })))}
                            </div>
                            <SharedFields formData={form} setter={setForm} />
                            <StudentOnlyFields formData={form} setter={setForm} />
                    ...
                    <div className="relative bg-white rounded-2xl shadow-2xl p-7 w-full max-w-lg z-10 border border-zinc-100 animate-in slide-in-from-bottom-4 duration-200">
                        <div className="flex items-center justify-between mb-5">
                            <div>
                                ...
                                <p className="text-sm text-zinc-400 mt-0.5">Updating <span className="font-medium text-zinc-700">{showEdit.full_name}</span></p>
                            </div>
                        ...
                        <form onSubmit={handleUpdate} className="space-y-4">
                            ...
                            <div>
                                {renderLabel("Role")}
                                {renderRoleRadios(editForm.role, (e) => setEditForm((p) => ({ ...p, role: e.target.value })))}
                            </div>
                            <SharedFields formData={editForm} setter={setEditForm} />
                            <StudentOnlyFields formData={editForm} setter={setEditForm} />
                            {renderDetailRow("Password Hash", editForm.password_hash)}
